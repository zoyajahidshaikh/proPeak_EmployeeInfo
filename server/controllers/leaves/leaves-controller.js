...
exports.leaveApplicationSave = async (request, response) => {
    try {
        ...
        if (config.prodMode === "ON") {
            const managerDetails = await userModel.findOne({ "_id": loggedInUserId }, { "reportingManagerId": 1 });
            ...
            if (manager) {
                ...
                if (emailResponse.response) {
                    logInfo(emailResponse, `Error occurred while sending email to ${toEmail}`);
                } else {
                    logInfo(`An e-mail has been sent to ${toEmail} with further instructions.`);
                }
            }
        } else {
