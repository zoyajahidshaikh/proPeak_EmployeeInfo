...
exports.categories_post = (async (req, res) => {
  ...
  if (req.body._id) {
    ...
    //      let company = cachedData.filter((element, indx)=>{
    //            if(element._id == req.body._id){
    //               return element;
    //            }
    //      });

    //     var res = getCategoryById(company, req.body);
    //     res.json(res);
    //     return;
    //   }
    // }
      ...
      .then((result) => {
        ...
        fields.filter((field) => {
          if(oldResult[field] !== result[field]){
            audit.insertAuditLog(oldResult[field], result.id, 'Category', field, result[field], userIdToken, '');
          }
        })
    ...
    .then((result) => {
      ...
      fields.filter((field) => {
        ...
        audit.insertAuditLog('', result.id, 'Category', field, result[field], userIdToken,'');
      })
...
exports.categories_put = ((req, res) => {
  ...
  Category.findOneAndUpdate({ _id: req.params.id }, updatedCategory, { context: 'query' })
    .then((oldResult) => {
        ...
        .then((newResult) => {
          ...
          fields.filter((field) => {
            if(oldResult[field] !== newResult[field]){
              audit.insertAuditLog(oldResult[field], newResult.id, 'Category', field, newResult[field], userIdToken,'');
            }
          })
          res.json({
            success: true,
...
exports.categories_delete = ((req, res) => {
  ...
  Category.findOneAndRemove({_id: req.body.id})
    .then((result) => {
      ...
      audit.insertAuditLog(result[field], result.id, 'Category', field, '', userIdToken,'');
