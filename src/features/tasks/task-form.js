...
export default class TaskForm extends React.Component {
    ...
    async getProjectTasks() {
        let { tasks, taskErr } = await taskservice.getProjectTasks(this.props.projectParamsId);
        if (taskErr) {
            this.setState({
                message: taskErr,
                //updatedTime:dateUtil.getTime()
    ...
    formatTasks = (task) => {
        ...
        let editAll = false;
        editAll = validate.validateEntitlements(accessRights, this.props.projectId, 'Task', 'edit all');
        ...
        if (getRole === "user" && !editAll) {
            ...
            if (task.userId === undefined || task.userId === null || task.userId === "") {
                ...
                let userName = "";
                userName = this.state.project && this.state.project.projectUsers.filter((u) => {
                    return u.userId === task.userId
                })
                task.hiddenUserId = (userName && userName.length > 0) ? userName[0].name : "";
            }
        }

        let taskMessages = task.messages && task.messages.filter((m) => {
            ...
            if (this.state.project) {
                ...
                if (this.state.project.projectUsers.length > 0) {
                    for (let x = 0; x < this.state.project.projectUsers.length; x++) {
                        if (this.state.project.projectUsers[x].name !== undefined && this.state.project.projectUsers[x].name !== null) {
                            userGroupsWithUsers.push({ id: this.state.project.projectUsers[x].userId, name: this.state.project.projectUsers[x].name });
                        }
                    }
                }
                ...
                if (this.state.project.userGroups.length > 0) {
                    for (let x = 0; x < this.state.project.userGroups.length; x++) {
                        userGroupsWithUsers.push({ id: this.state.project.userGroups[x].groupId, name: this.state.project.userGroups[x].groupName });
                    }
                }

                let projUsers = userGroupsWithUsers.filter((u) => {
                    if (this.state.task.assignUsers !== undefined && this.state.task.assignUsers.length > 0) {
                    ...
                    if (this.state.task.hiddenUserId !== undefined && this.state.task.hiddenUserId !== null && this.state.task.hiddenUserId !== '') {
                        let usersId = "";
                        if (this.state.task.hiddenUserId.toLowerCase().replace(/ +/g, "") === u.name.toLowerCase().replace(/ +/g, "")) {
                            usersId = u.id;
                        }
                ...
                for (let i = 0; i < projUsers.length; i++) {
                    if (projUsers[i].name !== undefined && projUsers[i].name !== null) {
                        if (projUsers[i].name.toLowerCase().indexOf(name1) > -1) {
                            projectUserD.push(<li key={projUsers[i].id} style={{ cursor: "pointer", marginLeft: "-20px" }}
                                onClick={this.addAssignUser.bind(this, projUsers[i].id)} id={projUsers[i].id}
                                value={projUsers[i].id}>{projUsers[i].name}</li>);
                        }
                    }
                }
            }
    ...
    validateField(fieldName, value) {
        ...
        switch (fieldName) {
            ...
            case 'description':
                ...
                fieldValidationErrors.description = descriptionValid ? '' : ' Please fill the';
                break;
            case 'storyPoint':
                storyPointValid = value.length !== 0;
                fieldValidationErrors.storyPoint = storyPointValid ? '' : ' Please fill the';
                break;
            case 'taskType':
                taskTypeValid = value.length !== 0;
                fieldValidationErrors.taskType = taskTypeValid ? '' : ' Please fill the';
                break;
            // case 'startDate':
    ...
    onKeyUp = (e) => {
        if (e.which === 188) {
            ...
            if ((tagsArr && Array.isArray(tagsArr)) && (tagsArr[0] === "" || tagsArr[0] === ",")) {
                tagsArr = [];
            }
    ...
    onDeleteTag = (tags) => {
        var tag = this.state.task.tag.split(',').filter((t) => {
            return (t !== tags);
        }).join(',');
    ...
    onSubmit = (e) => {
        ...
        let depTask = pTasks && pTasks.filter((dTask) => {
            return dTask.title === hiddenDepId;
        });
        let taskUser = project.projectUsers && project.projectUsers.filter((pUser) => {
        ...
        if (hiddenDepId !== '' && hiddenUserId !== '') {
            if (depTask.length !== 0 && taskUser.length === 0) {
                ...
                if (this.state.task.title !== "" && this.state.task.description !== ""
                    && this.state.task.status !== "" && this.state.task.storyPoint !== "") {
            ...
            if (this.state.task.title !== "" && this.state.task.description !== "" && this.state.task.taskType !== ""
                && this.state.task.priority !== "" && this.state.task.status !== "" && this.state.task.storyPoint !== "") {
                this.submitCheck(this.state.task);
    ...
    onTaskSubmit = (task) => {
        ...
        if (task.hiddenDepId === undefined || task.hiddenDepId === null || task.hiddenDepId === "") {
            task.depId = "";
        } else {
            let pTaskId = this.state.pTasks && this.state.pTasks.filter((pId) => {
        ...
        if (getRole === "user" && !editAll) {
            var users = this.state.project && this.state.project.projectUsers.filter((u) => {
                return getId === u.userId;
            })[0];
            task.userId = users.userId;
            task.hiddenUserId = users && users.name;
        } else {
            if (task.hiddenUserId === undefined || task.hiddenUserId === null || task.hiddenUserId === "") {
                task.userId = "";
            } else {
                let uTaskId = this.state.project && this.state.project.projectUsers.filter((uId) => {
        ...
        if (this.state.taskId || this.state.taskParamsId) {
            ...
            this.editTask(task, id, email, projectName, ownerEmail, userName, depTaskTitle);
        } else {
            if (task.status === "completed") {
                task.completed = true;
                task.category = "completed"
            ...
            if (task.assignUsers.length > 0) {
                for (let i = 0; i < task.assignUsers.length; i++) {
                    let user = (this.state.users.length > 0) && this.state.users.filter((u) => {
                    ...
                    let matchUserGroup = (this.state.project && this.state.project.userGroups.length > 0) && this.state.project.userGroups.filter((g) => {
                        return task.assignUsers[i] === g.groupId;
                    })
                    let groupMembers = (matchUserGroup.length > 0) ? matchUserGroup[0].groupMembers : [];
                    if (groupMembers.length > 0) {
                        for (let k = 0; k < groupMembers.length; k++) {
                            multiUsers.push({ id: groupMembers[k].id, name: groupMembers[k].name });
                        }
                    }
                }
            }
            // console.log('task in Add',task);
            this.addTask(task, email, userName, projectName, depTaskTitle, multiUsers);
        }
    }

    addTask = async (task, email, userName, projectName, depTaskTitle, multiUsers) => {
    ...
    editTask = async (task, id, email, projectName, ownerEmail, userName, depTaskTitle) => {
    ...
    onDeleteAssignUsers(tag) {
        let userI = this.state.usersAndGroups.filter((u) => {
            return u.name.toLowerCase().replace(/ +/g, "") === tag.toLowerCase().replace(/ +/g, "");
        });
    ...
    onToggleNewSubTask(taskId, taskSubTaskId) {
        let task = this.state.tasks && this.state.tasks.filter((t) => {
            return taskId === t._id
        })
    ...
    async onSubTaskEdit(subtask) {
        ...
        let taskd1 = this.state.tasks && this.state.tasks.filter((t1) => {
    ...
    handlesubTaskInputChange(id, taskId, e) {
        ...
        let task = (this.state.tasks !== undefined) && this.state.tasks.filter((t) => {
            return t._id === taskId
        });
        if (task.length > 0) {
    ...
    checkSubmit() {
        if (this.state.task.startDate !== "" && this.state.task.endDate !== "") {
            if (Date.parse(this.state.task.startDate) > Date.parse(this.state.task.endDate)) {
                this.setState({ submitDisabled: true, errMessage: 'Start Date is Greater Than End Date' });
            }
            else {
    ...
    render() {
        ...
        var { title, description, tag, status, storyPoint, startDate, endDate, hiddenDepId, hiddenUserId,
            taskType, priority, allowMultipleUsers, selectUsers, assignUsers } = this.state.task;
        var { currentUser, pTasks, taskTypes, checkMsg, taskPriorities, project } = this.state;
        ...
        let formattedDesc = (description && description.length > 0) ? description.split("\n").map((t) => <p key={t}>{t}</p>) : "";




        var assignusers = (assignUsers !== undefined && assignUsers.length > 0) && assignUsers.map((tag) => {
        ...
        let task = [];
        if (this.state.taskId) {
            task = (this.state.tasks !== undefined) && this.state.tasks.filter((t) => {
                return t._id === this.state.taskId
            });
        }
        ...
        let uCreatedOn = (task && task.length > 0) ? dateUtil.DateToLongString(task[0].createdOn) : '';
        if (this.state.taskId || this.state.taskParamsId) {
            if (task.length > 0 && task[0].subtasks && task[0].subtasks.length > 0) {
                task[0].subtasks.sort((a, b) => (a.sequence - b.sequence));
            }

            var subTasksView = (task.length > 0 && task[0].subtasks) && task[0].subtasks.map((subTask) => {
                var edit = subTask.edit;
                var add = subTask.add;
                var subTaskchecked = subTask.completed ? "checked" : "";
                let taskData = this.state.tasks &&
                    this.state.tasks.filter((f) => { return f.subtaskId === subTask._id })
                let taskId = taskData.length > 0 ? taskData[0]._id : '';
                ...
                for (var i = 0; i < task[0].subtasks.length; i++) {
                    if (task[0].subtasks[i]._id !== subTask._id) {
                        tsubtasks.push(
                            { id: task[0].subtasks[i]._id, title: task[0].subtasks[i].title }
                        );
                    }
                }
                                ...
                                <form >
                                        ...
                                        <div className="col-sm-6 col-md-9">
                                            <div className="form-group">
                                                ...
                                                <input type="text" className="form-control"
                                                    ...
                                                    value={subTask.title}
                                                    onChange={this.state.taskParamsId ? this.handlesubTaskInputChange.bind(this, subTask._id, this.state.taskParamsId)
                                                        : this.handlesubTaskInputChange.bind(this, subTask._id, this.state.taskId)}
                                    ...
                                    <div className="row">
                                        <div className="col-sm-6 col-md-6">
                                            <div className="form-group">
                                                <label htmlFor="Select User" style={labelStyle}>Select Memeber</label>
                                                <span style={{ color: 'red' }}>*</span>
                                                ...
                                                <datalist id="usersData" >
                                                    {
                                                        this.state.project && this.state.project.projectUsers.map((u) => {
                                        ...
                                        <div className="col-sm-6 col-md-6">
                                            <div className="form-group">
                                                ...
                                                <span style={{ color: 'red' }}>*</span>
                                                <input type="number" name="subtaskStoryPoint" className="form-control"
                                    ...
                                    <div className="row">
                                        <div className="col-sm-12 col-md-12">
                                            <div className="form-group">
                                                ...
                                                <input type="text" value={subTask.subtaskhiddenDepName} list="data1"
                                                    onChange={this.state.taskParamsId ? this.handlesubTaskInputChange.bind(this, subTask._id, this.state.taskParamsId)
                                                        : this.handlesubTaskInputChange.bind(this, subTask._id, this.state.taskId)}
                                    ...
                                    <div className="row">
                                        <div className="col-sm-6 col-md-6">
                                            <input type="submit" value="Save" className="btn btn-info btn-block" onClick={this.onSubmitSubtask.bind(this, subTask)} disabled={!subTask.title} />
                                        </div>
                                        <div className="col-sm-6 col-md-6">
                                            <button value='cancel' className="btn btn-secondary btn-block" onClick={this.onCloseSubtask.bind(this, this.state.taskId, subTask._id)}>
                                                Cancel</button>
                                        </div >
                                    </div>
                                </form>
                                    ...
                                    {edit ?
                                        <div className="container">
                                            <form >
                                                ...
                                                <div className="row">
                                                    ...
                                                    <div className="col-sm-6 col-md-6">
                                                        <button value='cancel' className="btn btn-info btn-block" onClick={this.onCloseSubtask.bind(this, this.state.taskId, subTask._id)}>
                                                            Cancel</button>
                                                    </div >
                                                </div>
                                            </form>
                                        ...
                                        <span>
                                            ...
                                            <input className="" type="checkbox" checked={subTaskchecked}
                                                onChange={this.state.taskParamsId ? this.onToggleSubTask.bind(this, subTask, this.state.taskParamsId) : this.props.onToggleSubTask.bind(this, subTask, this.state.taskId)} />
                                            &nbsp;
                                            <span title={subTask.title} className="show__overflow_dots subtask_title">{subTask.title}</span>
                                        </span>
                                    }

                                </span>
                                <span className="subtask-actions" >
                                    <span onClick={this.state.taskParamsId ? this.changeSubtaskSequence.bind(this, this.state.taskParamsId, subTask._id, subTask.sequence, 'up') : this.props.changeSubtaskSequence.bind(this, this.props.taskId, subTask._id, subTask.sequence, 'up')}>
                                        <i className="fas fa-arrow-up"></i>
                                    </span>
                                    <span onClick={this.state.taskParamsId ? this.changeSubtaskSequence.bind(this, this.state.taskParamsId, subTask._id, subTask.sequence, 'down') : this.props.changeSubtaskSequence.bind(this, this.props.taskId, subTask._id, subTask.sequence, 'down')}>
                                        <i className="fas fa-arrow-down"></i>
                                    </span>
                                    {
                                        subTask.completed === false ?
                                            <span onClick={this.state.taskParamsId ? this.onTogglesubTaskEdit.bind(this, subTask._id, this.state.taskParamsId)
                                                :
                                                this.props.onTogglesubTaskEdit.bind(this, subTask._id, this.props.taskId)}>
                                    ...
                                    <span
                                        onClick={() => {
                                            ...
                                            this.state.taskParamsId ? this.onDeleteSubTask(subTask._id, this.state.taskParamsId) :
                                                this.props.onDeleteSubTask(subTask._id, this.state.taskId)
        ...
        return (
            <div style={{ border: "1px solid #EEE" }}>
                {!this.state.taskParamsId ? <span onClick={this.props.closeTask} className="float-right mr-3 mt-2">
                    <i className="fas fa-times close"></i>
                </span> : ''}
                {this.state.taskParamsId ? <h3 className="project-title"><Link to={'/project/tasks/' + projectLinkId} className="">{projectTitle}</Link></h3> : ''}
                ...
                {this.state.taskId || this.state.taskParamsId ?
                    <div className="nav nav-tabs nav-fill" role="tablist">
                        ...
                        <a className="nav-item nav-link active" href="#editTask" aria-controls="editTask"
                            data-height="true" role="tab" data-toggle="tab" onClick={this.handleTabClick.bind(this, 'editTask')}>Task</a>

                        <a className="nav-item nav-link " href="#messages" aria-controls="messages" role="tab"
                            data-height="true" data-toggle="tab" onClick={this.handleTabClick.bind(this, 'messages')}>Messages <span className="text-warning">({this.state.taskParamsId ? (this.state.messages !== undefined) && this.state.messages.length : (this.props.messages !== undefined) && this.props.messages.length})</span></a>


                        <a className="nav-item nav-link " href="#uploads" aria-controls="uploads" role="tab"
                            data-height="true" data-toggle="tab" onClick={this.handleTabClick.bind(this, 'uploads')} >Attachment(s) <span className="text-warning">({this.state.taskParamsId ? (this.state.uploadFiles !== undefined) && this.state.uploadFiles.length : (this.props.uploadFiles !== undefined) && this.props.uploadFiles.length})</span></a>
                    </div> : ''}
                ...
                <div className="tab-content">
                        ...
                        <div className="form-wrapper" >
                            ...
                            <form onSubmit={this.onSubmit} id="task">
                                ...
                                <div className="row">
                                    ...
                                    <div className="col-sm-6">
                                        <div className="form-group">
                                            <label htmlFor="Status" style={labelStyle}>Status</label><span style={{ color: 'red' }}>*</span>
                                            <select value={status} onChange={this.handleInputChange} name="status" className="form-control">
                                    ...
                                    <div className="col-sm-6">
                                        <div className="form-group">
                                            <label htmlFor="Dependency Task" style={labelStyle}>Task Dependency</label>
                                            <input type="text" value={hiddenDepId} list="data" onChange={this.handleInputChange}
                                                name="hiddenDepId" className="form-control" autoComplete="off" placeholder="Task Dependency" />
                                            <datalist id="data" >
                                                {
                                                    pTasks && pTasks.map((t) => {
                                                        return <option key={t.id} data-value={t.id}>{t.title}</option>
                                    ...
                                    <div className="col-sm-6">
                                        <div className="form-group">
                                            <label htmlFor="StoryPoint" style={labelStyle}>Story Point ({config.storyPoint})</label>
                                            <span style={{ color: 'red' }}>*</span>
                                            <input type="number" name="storyPoint" className="form-control"
                                                placeholder="Story Point" min="1"
                                                value={storyPoint}
                                    ...
                                    <div className="col-sm-6">
                                        <div className="input-group">
                                            ...
                                            {(this.state.task.status === 'inprogress' || this.state.task.status === 'completed') ? <span style={{ color: 'red' }}>*</span> : ''}

                                            <Calendar width='235px' height='225px' className="form-control"
                                    ...
                                    <div className="col-sm-6">
                                        <div className="form-group">
                                            <label htmlFor="Task Type" style={labelStyle}>Task Type</label><span style={{ color: 'red' }}>*</span>
                                            <select value={taskType} onChange={this.handleInputChange} name="taskType" className="form-control">
                                                <option value="" disabled>Select Task Type</option>
                                                {
                                                    taskTypes.map((t) => {
                                                        return <option key={t.title}
                                                            value={t.title}>{t.displayName}</option>;
                                                    })
                                                }
                                            </select>
                                        </div>
                                    ...
                                    <div className="col-sm-6">
                                        <div className="form-group">
                                            <label htmlFor="Select User" style={labelStyle}>Select Member:</label>
                                            {currentUser && !editAll ? "" : <span title="Add users to project" onClick={() => { this.onShowAddUserModal(this.state.taskId) }}
                                                style={{ marginLeft: "10px", color: '#FF9800' }}>
                                                <i className="fas fa-plus-square"></i>
                                            </span>}
                                            {currentUser && !editAll ?
                                                <input type="text" value={hiddenUserId} list="usersData" onChange={this.handleInputChange}
                                                    name="hiddenUserId" className="form-control" autoComplete="off" disabled />
                                                :
                                                <input type="text" value={hiddenUserId} list="usersData" onChange={this.handleInputChange}
                                            ...
                                            <datalist id="usersData" >
                                                {
                                                    project && project.projectUsers.map((u) => {
                                                        return <option key={u.userId}
                                                            data-value={u.userId}>{u.name}</option>;
                                                    })
                                                }
                                    ...
                                    <div className="col-sm-6">
                                        <div className="form-group">
                                            <label htmlFor="Task Priority" style={labelStyle}>Task Priority</label><span style={{ color: 'red' }}>*</span>
                                            <select value={priority} onChange={this.handleInputChange} name="priority" className="form-control">
                                                <option value="" disabled>Select Task Priority</option>
                                                {
                                                    taskPriorities && taskPriorities.map((t) => {
                                                        return <option key={t._id}
                                                            value={t.priority}>{t.displayName}</option>;
                                                    })
                                                }
                                            </select>
                                        </div>
                                ...
                                <div className="row">
                                    <div className="col-sm-12">
                                        <div className="form-group">
                                            ...
                                            {this.state.taskId ? <span className="pull-right" onClick={() => { this.onShowTaskModal(this.state.taskId) }} title="Show Description"
                                                style={{ cursor: "pointer" }}>
                                                <small>&nbsp; <i className="fas fa-external-link-alt"></i></small></span> : ""}
                                            <textarea name="description" className="form-control"
                                                value={description}
                                                placeholder="Description"
                                                onChange={this.handleInputChange} style={{ height: '75px' }} />
                                        </div>
                                    </div>
                                </div>

                                {!this.state.taskId && !this.state.taskParamsId && (userRole !== "user" || editAll === true) ?
                                    <div className="row">

                                        <div className="col-sm-6">
                                            <div className="form-group">
                                                <label style={{ fontSize: "small", marginRight: "7px", textTransform: "capitalize" }}>Replicate for multiple Members</label>
                                                &nbsp;
                                                <input type='checkbox' name="allowMultipleUsers" onChange={this.handleInputChange} checked={allowMultipleUsers} />
                                            </div>
                                        </div>
                                        ...
                                        {allowMultipleUsers === true ?
                                            <div className="col-sm-6">
                                                <div className="form-group">
                                                    <label htmlFor="Select Users" style={labelStyle}>Select Members</label>
                                                    <input type="text" value={selectUsers} className="form-control" onKeyDown={this.onUserKeyPress.bind(this)}
                                                        onChange={this.handleInputChange} placeholder="Select Members"
                                                        name="selectUsers" autoComplete="off" style={{ position: 'relative' }} />
                                                    <div style={{ position: 'absolute', left: '16px', top: '64px', width: '92%', border: "1px solid #ccc4c4", height: "100px", overflowY: "auto", background: '#fff', zIndex: 50 }}
                                ...
                                <div className="row">
                                    <div className="col-sm-12">
                                        <div className="form-group">
                                            ...
                                            <input type="text" name="tag" className="form-control" placeholder="Press comma to add Tag"
                                                onKeyUp={(e) => this.onKeyUp(e)}
                                            />
                                            {tags && tags.length > 0 ?
                                                <div style={{ height: "45px", overflowY: 'auto', overflowX: 'hidden' }}>
                                                    <i className="fas fa-tags mytags"></i> &nbsp; {tags}
                                                </div>
                                                :
                                                <span><i className="fas fa-tags mytags"></i> &nbsp; <small className="text-muted">Tags...</small></span>
                                            }
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div className="row mb-5">
                                    <div className="col-sm-12">
                                        <input type="submit" className="btn btn-info btn-block"
                                            ...
                                            // }
                                            disabled={(this.state.task.status === 'inprogress' || this.state.task.status === 'completed') ?
                                                !(title && description && startDate && endDate && !this.state.submitDisabled)
                                                :
                                                !(title && description && !this.state.submitDisabled)
                                            }
                                        />
                                    </div>
                                </div>
                            ...
                            {this.state.taskId || this.state.taskParamsId ?
                                <div><span style={{ marginLeft: '6px', fontSize: '15px', fontWeight: 'bold' }} >
                                    Sub Tasks
                                </span>
                                    <span title='New Subtask' onClick={this.state.taskParamsId ? this.onToggleNewSubTask.bind(this, this.state.taskParamsId, -1) :
                                        this.props.onNewSubTask && this.props.onNewSubTask.bind(this, this.state.taskId, taskUserId, -1, true)}>
                    ...
                    {this.state.taskId ? <ModalSmall show={this.state.isOpen} onClose={this.toggleModal} title="Task Description">
                        {formattedDesc}
                    </ModalSmall> : ''}

                    <ModalSmall show={this.state.isAddUserModalOpen} onClose={this.toggleAddUserModal} title="Add users to project">
                        <AddProjectUser projectId={this.state.projectId} users={this.state.users} userNameToId={this.state.userNameToId} user={this.state.user} getProjectTasks={this.props.getProjectTasks} project={this.state.project} />
                    </ModalSmall>

                    <div role="tabpanel" className="tab-pane" id="messages">
                        <MessageList messages={this.state.messages}
                            projectId={!this.state.projectId ? this.state.projectParamsId : this.state.projectId}
                            deleteMessageTask={this.state.taskParamsId ? this.state.deleteMessageTask : this.props.deleteMessageTask}
                            taskId={!this.state.taskId ? this.state.taskParamsId : this.state.taskId}
                            users={this.props.context.state.users}
                            addTaskMsg={this.state.taskParamsId ? this.addTaskMsg : this.props.addTaskMsg}
                            user={this.props.context.state.user} />
                    </div>
                    ...
                    <div role="tabpanel" className="tab-pane" id="uploads">
                        <UploadFile
                            ...
                            projectId={!this.state.projectId ? this.state.projectParamsId : this.state.projectId}
                            taskId={!this.state.taskId ? this.state.taskParamsId : this.state.taskId}
                            addUploadTaskFile={this.state.taskParamsId ? this.addUploadTaskFile : this.props.addUploadTaskFile}
                            deleteTaskFileById={this.state.taskParamsId ? this.deleteTaskFileById : this.props.deleteTaskFileById} />
                    </div>
