...
class LeaveApplication extends Component {
    ...
    async setValues() {
        ...
        if (err) {
            ...
            document.getElementById("leaveType").value = response.data.leaveDetails.leaveType
        }
    }
    handleChange(e) {
    ...
    dateCheck() {
        if (this.state.fromDate !== "" && this.state.toDate !== "") {
            if (Date.parse(this.state.fromDate) > Date.parse(this.state.toDate)) {
                ...
                this.setState({ submitDisabled: true, errorMessage: 'From Date is Greater Than To Date', duration: "" });
            }
            else {
                ...
                this.setState({ submitDisabled: false, errorMessage: '', workingDays: diffDays });
            }
        }
    }
    ...
    onCheckEligibility(event) {
        ...
        let datavalue = dropdwn.options[dropdwn.selectedIndex].getAttribute("data-value");
    ...
    async checkEligibility(leaveApplication) {
        // console.log("leaveApplication", leaveApplication);
        let { response, err } = await leaveApplicationService.checkEligibility(leaveApplication);
        this.setState({
            isLoaded: true
        });
        if (err) {
            ...
            if (response.data.leaveValidationResult.isEligible) {
                this.setState({
                    ...this.state,
                    // leaveType: "",
                    balanceMessage: "The balance is : " + response.data.leaveValidationResult.balance,
                    isElegible: true
                })
            }
            else {
    ...
    render() {
        ...
        leaveTypeDrpDown.push(<option value='' key="mod">Select Leave Type</option>)
        this.state.leaveTypes.forEach(function (module, i) {
            leaveTypeDrpDown.push(<option value={module.LeaveTypeName} data-value={module.id} key={"mod_" + i}>{module.LeaveTypeName}</option>)
        })
        LeaveCategoryArray.push(<option value='' key="mod">Select Leave Category</option>)
        ...
        if (this.state.holidayList.length > 0) {
            this.state.holidayList.forEach(function (module, i) {
                HolidayList.push(<tr ><td key={"mod_" + i} >{module.date}</td><td key={"mod_" + i + 1} >{module.holiday}</td></tr>)
            })
        } else {
            HolidayList.push(<tr><td colSpan="2">No records found!!!</td></tr>)
        }
        const fontStyle = {
            fontSize: "small",
        };
        let { leaveType, workingDays, fromDate, toDate, isElegible, leaveCategory, leaveReason } = this.state;
        return (
            <div>
                {this.state.isLoaded ?
                    <div className="logo">
                    ...
                    <div className="row">
                        <div className={this.state.isHolidayList ? taskClass : noTaskClass}>
                        <h4 className="project-title">Apply for Leave</h4>
                            ...
                            <div className="row">
                                <div className="col-sm-12">
                                    ...
                                    <a href="" className="text-info  float-right" data-toggle="modal" data-target="#holidayCenter" onClick={this.getAllHolidays}> Holidays in current year </a>
                                </div>
                            </div>
                                            ...
                                            <div className="row">
                                                    <div className="col-sm-3">
                                                        <div className="form-group">
                                                            ...
                                                            <select value={leaveType} id="leaveType" name="leaveType" className="form-control rounded-0" onChange={this.handleChange}>
                                                                {leaveTypeDrpDown}
                                                            </select>
                                                        </div>
                                                    </div>
                                                    <div className="col-sm-3">
                                                        <div className="form-group">
                                                            ...
                                                            <span style={{ color: "red" }}>*</span>
                                                            <select value={leaveCategory} id="leaveCategory" name="leaveCategory" className="form-control rounded-0" onChange={this.handleChange}>
                                                    ...
                                                    <div className="col-sm-3">
                                                        <div className="input-group">
                                                            <div><label htmlFor="fromDate">From Date</label> <span style={{ color: "red" }}>*</span></div> 
                                                            ...
                                                            <Calendar width='267px' height='225px' className="form-control rounded-0"
                                                                dateformat={'YYYY-MM-DD'}
                                                                selectedDate={this.state.fromDate}
                                                                dateUpdate={this.dateUpdate.bind(this, 'fromDate')}
                                                                id="fromDate" calendarModalId="fromDateModal"
                                                            />
                                                        </div>
                                                    </div>
                                                    <div className="col-sm-3">
                                                        <div className="input-group">
                                                            <div><label htmlFor="toDate">To Date</label> <span style={{ color: "red" }}>*</span></div> 
                                                        
                                                            <Calendar width='267px' height='225px' className="form-control rounded-0"
                                                                dateformat={'YYYY-MM-DD'}
                                                                selectedDate={this.state.toDate}
                                                                dateUpdate={this.dateUpdate.bind(this, 'toDate')}
                                                                id="toDate" calendarModalId="toDateModal"
                                                            />
                                                        </div>
                                                    </div>
                                                </div>
                                                <div className="row">
                                                    <div className="col-sm-3">
                                                        <div className="form-group">
                                                            ...
                                                            <label name="workingDays" id="workingDays">{this.state.workingDays} </label>
                                                ...
                                                {this.state.leaveType === 'Sick Leave' || this.state.leaveType === 'Casual Leave' ?
                                                    <div className="col-sm-3">
                                                        <div className="form-group">
                                                            <span>
                                                                <a className="linkstyle" style={{ cursor: "pointer" }} id="btnCheckEligibility" disabled={!(leaveType && workingDays && fromDate && toDate)} onClick={this.onCheckEligibility} ><span>Check Eligiblity</span></a>
                                                            </span>
                                                            <br />
                                                    ...
                                                    <div className="col-sm-6">
                                                        <div className="form-group">
                                                            ...
                                                            {this.state.leaveType === "Comp Off" ? <span style={{ color: "red" }}>*</span> : ''}
                                                            <textarea className="form-control rounded-0" rows="3" name="leaveReason" id="leaveReason" onChange={this.handleChange} value={this.state.leaveReason}></textarea>
                                                        </div>
                                                    </div>
                        ...
                        {this.state.isHolidayList ?
                            <div className="col-sm-4">
                              ...
                              <h4 className="project-title">Holiday List
                                    <span onClick={this.closeHolidayList} className="float-right">
